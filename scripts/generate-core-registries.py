...
tests = [
    ...
    ("AT-M1-LIFECYCLE-RECOVERY", "M1", "A surviving process marker is recorded as an unclean recovery event; a normal exit clears it, and CLI config checks are read-only.", "cargo test -p personal-agent-platform lifecycle_marker && cargo test -p personal-agent-core config_check", "automated", "passed"),
    ...
    ("AT-M2-SIDECAR-AUTH", "M2", "OpenCode binds loopback only with per-run auth and exact-version compatibility.", "cargo test -p personal-agent-runtime", "automated", "passed"),
    ("AT-M2-OPENAPI-FINGERPRINT", "M2", "Startup rejects the pinned sidecar if its authenticated OpenAPI contract differs from the reviewed release fingerprint.", "cargo test -p personal-agent-runtime bundled_sidecar", "automated", "passed"),
    ("AT-M2-GENERATED-CLIENT", "M2", "The generated authenticated stable client discovers project-scoped models and creates, resumes, forks, compacts or rejects, and aborts isolated sidecar sessions.", "cargo test -p personal-agent-runtime bundled_sidecar", "automated", "passed"),
    ("AT-M2-FAKE-TOOL-TURN", "M2", "The deterministic fake provider streams normalized response and tool lifecycle events through a terminal event.", "cargo test -p personal-agent-runtime fake_runtime_streams_a_deterministic_tool_turn_to_terminal", "automated", "passed"),
    ("AT-M2-SYNTHETIC-PROVIDER", "M2", "The stable sidecar loads an isolated OpenAI-compatible provider and completes an advertised native-gateway tool turn with normalized streaming and terminal state.", "cargo test -p personal-agent-runtime isolated_sidecar_streams_a_synthetic_provider_tool_turn", "automated", "passed"),
    ("AT-M2-SAFETY-PLUGIN", "M2", "The packaged OpenCode plugin has a loader-compatible export shape, fails unknown permission callbacks closed, exposes reviewed coding built-ins under granular permissions, preauthorizes each call through the registered-session native boundary, and surfaces reviewed permission/question events before the turn completes; the real sidecar fixture must complete an actual workspace file edit.", "bun --cwd packages/opencode-plugin test && bun --cwd apps/desktop test App.voice.test.tsx && cargo test -p personal-agent-runtime isolated_sidecar_streams_a_synthetic_provider_tool_turn", "automated", "passed"),
    ("AT-M2-PROFILE-ISOLATION", "M2", "Version probes and the running sidecar use an application-owned private profile and do not inherit ambient provider keys, OpenCode overrides, proxy settings, or package-manager configuration.", "cargo test -p personal-agent-runtime sidecar_profile_is_private_and_excludes_ambient_credentials", "automated", "passed"),
    ("AT-M2-NATIVE-TOOL-BRIDGE", "M2", "The sidecar plugin can use the native bridge only with its ephemeral bearer token and an exact registered session/workspace match; coding preauthorization rejects escaped paths and destructive shell bypasses, while accepted native tools traverse policy, filtering, verification, and audit.", "cargo test -p personal-agent-runtime native_tool_bridge_requires_authentication_and_registered_session_scope && cargo test -p personal-agent-runtime isolated_sidecar_streams_a_synthetic_provider_tool_turn", "automated", "passed"),
    ...
    ("AT-M3-CONVERSATION-STATES", "M3", "Typed, voice, sleep, mute, quiet, stop, follow-up, guest, and project states stay distinct.", "cargo test -p personal-agent-core conversation", "automated", "passed"),
    ("AT-M3-BARGE-IN", "M3", "Internal speaker stop is under 50 ms and end-to-end barge-in is under 100 ms.", "python scripts/verify-performance.py plus physical audio run", "hardware", "in_progress"),
    ...
    ("AT-M4-PROJECT-CONTEXT", "M4", "Opening, closing, and switching project context does not lose unrelated conversation state.", "cargo test -p personal-agent-core project_persona_and_model_switches_preserve_unrelated_contexts", "automated", "passed"),
    ...
    ("AT-M5-BROWSER-SAFE", "M5", "Isolated profile tasks work and malicious page instructions cannot cross data zones.", "cargo test -p personal-agent-browser", "automated", "passed"),
    ...
    ("AT-M5-ROLLBACK", "M5", "Checkpoints precede mutations and rollback snapshots current state before restoring.", "cargo test -p personal-agent-tools rollback", "automated", "passed"),
    ("AT-M6-DAG-RECOVERY", "M6", "A multi-hour DAG survives restart and provider failure without duplicate consequential actions.", "cargo test -p personal-agent-agent multi_hour_goal && cargo test -p personal-agent-storage durable_runtime_snapshots", "automated", "passed"),
    ("AT-M6-MEMORY-PROVENANCE", "M6", "Explicit facts are trusted, inference is queued, conflicts remain visible, and recalled text is not re-extracted.", "cargo test -p personal-agent-memory", "automated", "passed"),
    ("AT-M6-AUTOMATION-RECOVERY", "M6", "Missed runs follow policy, approvals suspend, and repeated failures pause an automation.", "cargo test -p personal-agent-automation && cargo test -p personal-agent-storage durable_runtime_snapshots", "automated", "passed"),
    ...
    ("AT-M7-REMOTE-PROTOCOL", "M7", "Third-party clients pair with fresh keys and cannot exceed negotiated capabilities.", "cargo test -p personal-agent-plugins remote", "automated", "passed"),
    ("AT-M7-COMPETITOR-COVERAGE", "M7", "Every adopted competitor capability is core or an installable official pack with an evaluation.", "python scripts/verify-packs.py && cargo test -p personal-agent-plugins official_pack_evaluations", "automated", "passed"),
    ("AT-M8-MIGRATION-DRY-RUN", "M8", "Synthetic and anonymized profiles dry-run and import idempotently without modifying source or copying plaintext secrets.", "cargo test -p personal-agent-migration -p personal-agent-storage -p personal-agent-core plus fixtures", "automated", "passed"),
    ("AT-M9-LIFECYCLE", "M9", "Fresh install, update, health rollback, export, uninstall, and optional data deletion pass on every platform.", "cargo test -p personal-agent-core release -p personal-agent-storage plus unsigned installer CI matrix", "mixed", "in_progress"),
    ("AT-M9-PERFORMANCE", "M9", "Latency and idle resource targets report p50, p95, max, and sample count.", "python scripts/verify-performance.py plus reference hardware", "hardware", "in_progress"),
    ("AT-M9-SECURITY", "M9", "No critical/high finding remains; SBOM, licenses, provenance, fuzzing, and red-team reports are complete.", "python scripts/security-gate.py && python scripts/verify-fuzz.py plus Security workflow", "mixed", "in_progress"),
]

cap_specs = [
    ("CAP-CONTRACTS","architecture","Versioned protobuf IPC/domain contracts and Draft 2020-12 schemas generate Rust and TypeScript types.", ["Jarvis"], "replaced-equivalently", "core", "M1", ["AT-M1-CONTRACT-DRIFT"], "passed"),
    ("CAP-STORAGE","storage","SQLCipher WAL event store, content-addressed blobs, transactional migrations, and rebuildable projections.", ["Jarvis"], "replaced-equivalently", "core", "M1", ["AT-M1-STORAGE-ENCRYPTED","AT-M1-EVENT-RECOVERY"], "passed"),
    ("CAP-CONFIG","configuration","Strict human-editable TOML backed by one canonical schema and keychain aliases.", ["Jarvis","Home Assistant"], "ported", "core", "M1", ["AT-M1-CONFIG-ROUNDTRIP"], "passed"),
    ("CAP-RUNTIME","agent-runtime","Provider-neutral AgentRuntime with pinned authenticated OpenCode sidecar, streaming, failover, and session lifecycle.", ["Jarvis","OpenCode","Codex","Claude Code","Gemini CLI"], "ported", "core", "M2", ["AT-M2-STREAMED-TURN","AT-M2-SIDECAR-AUTH","AT-M2-OPENAPI-FINGERPRINT","AT-M2-GENERATED-CLIENT","AT-M2-FAKE-TOOL-TURN","AT-M2-SYNTHETIC-PROVIDER","AT-M2-SAFETY-PLUGIN","AT-M2-PROFILE-ISOLATION","AT-M2-NATIVE-TOOL-BRIDGE"], "in_progress"),
    ...
    ("CAP-CONVERSATION","conversation","Persistent typed and voice conversations with projects, persona, follow-up, guest, sleep, mute, quiet, and stop states.", ["Jarvis","ChatGPT","Claude","Gemini","Alexa+"], "ported", "core", "M3", ["AT-M3-CONVERSATION-STATES"], "passed"),
    ...
    ("CAP-AGENT","agent-runtime","Durable goals, validated task DAGs, planner/executor/reviewer agents, bounded parallelism, recovery, steering, and verification.", ["OpenCode","Codex","Claude Code","OpenHands","Goose"], "new", "core", "M6", ["AT-M6-DAG-RECOVERY"], "passed"),
    ("CAP-MEMORY","memory","Working, episodic, semantic, procedural, project, and entity memory with provenance, review, conflict, expiry, FTS, and vectors.", ["Jarvis","OpenClaw","ChatGPT","Claude"], "strengthened", "core", "M6", ["AT-M6-MEMORY-PROVENANCE"], "passed"),
    ("CAP-AUTOMATION","automation","Cron, event, semantic, and heartbeat triggers with quiet hours, previous state, concurrency, missed runs, and failure pauses.", ["Jarvis","ChatGPT Tasks","Home Assistant","automation platforms"], "strengthened", "core", "M6", ["AT-M6-AUTOMATION-RECOVERY"], "passed"),
    ("CAP-EXTENSIONS","extensions","Agent Skills, experts, commands, MCP, OAuth connectors, signed plugins, WASI/process isolation, and proposal review.", ["Jarvis","OpenCode","Claude Code","Codex","Cline"], "strengthened", "core", "M7", ["AT-M7-PLUGIN-SAFETY","AT-M7-COMPETITOR-COVERAGE"], "passed"),
    ("CAP-CONNECTORS","connectors","Official productivity, communications, development, smart-home, media, cloud, travel, shopping, and creative packs.", ["ChatGPT","Claude","Gemini","Alexa+","Home Assistant"], "new", "official-pack", "M7", ["AT-M7-COMPETITOR-COVERAGE"], "passed"),
    ("CAP-RESEARCH","research","Cited multi-source research, contradiction reporting, saved projects, structured extraction, and source provenance.", ["ChatGPT","Perplexity","Gemini","Claude"], "new", "core", "M7", ["AT-M7-COMPETITOR-COVERAGE"], "passed"),
    ...
    ("CAP-REMOTE","remote","Optional secure, capability-negotiated third-party remote protocol with fresh pairing and revocation.", ["Jarvis","OpenCode"], "ported", "official-pack", "M7", ["AT-M7-REMOTE-PROTOCOL"], "passed"),
    ("CAP-MIGRATION","migration","Read-only discovery, dry-run, consented idempotent import, provenance, reports, and no plaintext secret transfer.", ["Jarvis"], "new", "core", "M8", ["AT-M8-MIGRATION-DRY-RUN"], "passed"),
    ("CAP-OPERATIONS","operations","Single-app lifecycle, watchdog, suspend recovery, signed multi-platform packaging, updates, rollback, export, uninstall, and support bundles.", ["Jarvis","Tauri"], "ported", "core", "M9", ["AT-M1-LIFECYCLE-RECOVERY","AT-M1-PLATFORM-BUILD","AT-M9-LIFECYCLE","AT-M9-PERFORMANCE"], "in_progress"),
]
...
rejections = [
    ...
    {"id":"REJ-010","behavior":"Trust OpenCode V1 before-hook mutation as the safety boundary","products":["OpenCode"],"reason":"Upstream reports show argument mutation/short-circuit gaps. Personal Agent never rewrites arguments in the hook or treats it as the sole boundary: reviewed coding tools layer pinned OpenCode permissions and canonical workspace scope with native preauthorization, while non-coding effects use gated native tools.","category":"security","decision":"replaced-equivalently","sources":["https://github.com/anomalyco/opencode/issues/42409","https://github.com/anomalyco/opencode/issues/32565"]},
]
